...
- WebView desktop UI (pywebview + Microsoft Edge WebView2 Runtime) is the default and only shipping UI as of Phase 1. The Overview page is fully migrated (Phase 1); the Timeline / Time Details page is migrated as a read-only page (Phase 2) and hardened for real-run reliability, refresh stability, and privacy boundary enforcement (Phase 2.1); Statistics/Export, Project Rules, and Settings/Privacy remain on the legacy Tkinter / CustomTkinter code pending per-page migration. Timeline editing, correction, and reclassification are not yet available in the WebView UI.
