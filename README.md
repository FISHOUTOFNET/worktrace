...
v0.2 is also evaluating an optional WebView UI spike. The existing Tkinter / CustomTkinter UI remains the default entry point and is not affected. The WebView is a spike only: it does not migrate any page, does not introduce an account, server, or network dependency, and does not change current functionality. You can try the WebView shell with `python -m worktrace.webview_main`; the default entry point remains `python -m worktrace.main`. See [`docs/ui-webview-migration.md`](docs/ui-webview-migration.md) for the Phase 0B scope, boundaries, and stop-loss conditions.
