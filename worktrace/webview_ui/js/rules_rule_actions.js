...
(function () {
    ...
    function installFDWorkProjectIdentityLifecycle() {
        ...
        function clearPickerTransient() {
            state.pickerRequestId = null; state.pickerEditorGeneration = null;
            state.pickerPending = false; state.pickerCanceling = false;
        }
        ...
        function callCancel(requestId) {
            if (!requestId || !App.bridge || typeof App.bridge.cancelFDWorkCasePicker !== "function") {
                return Promise.resolve({ ok: false, accepted: false });
            }
            return Promise.resolve(App.bridge.cancelFDWorkCasePicker(requestId)).catch(function () {
        ...
        function cancelPicker(detach) {
            ...
            if (detach) {
                clearPickerTransient(); refresh(); void callCancel(requestId); return Promise.resolve(true);
            }
            if (state.pickerCanceling) return Promise.resolve(true);
            state.pickerCanceling = true; showStatus("正在取消案件选择…", false); refresh();
            return callCancel(requestId).then(function (result) {
                ...
                clearPickerTransient(); showStatus("案件选择已取消", false); refresh(); return true;
            });
        }
        function reset() {
            if (state.pickerPending) cancelPicker(true);
            state.editorGeneration += 1;
            state.selectionProof = null; state.selectedLabel = ""; state.originalName = "";
            state.originalBound = false; state.editingProjectId = null; state.projectBusy = false;
            state.clearBindingOnSave = false; state.saveIntent = "local"; clearPickerTransient();
        ...
        function openPicker() {
            ...
            if (state.pickerPending) { void cancelPicker(false); return; }
            var generation = state.editorGeneration;
            var requestId = "rules-picker-" + generation + "-" + (++state.pickerCounter);
            state.pickerPending = true; state.pickerCanceling = false;
            state.pickerRequestId = requestId; state.pickerEditorGeneration = generation;
            showStatus("正在打开案件选择器…", false); refresh();
            Promise.resolve(baseBridge.openFDWorkCasePicker(requestId)).then(function (result) {
                ...
                if (!result || result.ok === false) {
                    clearPickerTransient(); showStatus(result && result.message || "打开案件选择器失败", true); refresh(); return;
                }
                showStatus(result.operation_status === "authentication_required" ? "请登录 FD Work" : "请在 FD Work 中选择案件", false);
                refresh();
            }).catch(function () {
                if (!currentPicker(requestId, generation)) return;
                clearPickerTransient(); showStatus("打开案件选择器失败", true); refresh();
            });
        }
        function clearSelection() {
            if (state.pickerPending) return;
            if (state.selectionProof) {
                state.selectionProof = null; state.selectedLabel = ""; state.clearBindingOnSave = false;
                if (state.originalBound) {
                    var input = projectNameInput(); if (input) input.value = state.originalName;
            ...
            if (state.originalBound) {
                ...
                showStatus(state.clearBindingOnSave ? "保存后取消 FD Work 关联" : "已关联 FD Work", false);
                refresh();
            }
        }
        function receivePickerResult(result) {
            ...
            if (result.ok !== true) {
                showStatus(result.error === "picker_canceled" ? "案件选择已取消" : "案件选择已失效", result.error !== "picker_canceled");
                refresh(); return true;
            }
            if (typeof result.selected_label !== "string" || !result.selected_label
                || typeof result.selection_token !== "string" || !result.selection_token) {
                showStatus("案件选择结果无效", true); refresh(); return false;
            }
            state.selectedLabel = result.selected_label.trim(); state.selectionProof = result.selection_token;
            state.clearBindingOnSave = false;
            var input = projectNameInput(); if (input) input.value = state.selectedLabel;
            showStatus("已选择 FD Work 案件", false); refresh(); return true;
        }
        function handleNameInput() {
            var input = projectNameInput(); var name = String(input && input.value || "").trim();
            if (state.selectionProof && name !== String(state.selectedLabel || "").trim()) {
                state.selectionProof = null; state.selectedLabel = "";
            }
        ...
        function buildSavePayload(localName, editing) {
            ...
            if (!name) { state.saveIntent = "local"; return { ok: false, error: "请输入项目名称" }; }
            if (!enabled()) {
                state.selectionProof = null; state.selectedLabel = ""; state.clearBindingOnSave = false; state.saveIntent = "local";
            ...
            if (state.selectionProof) {
                ...
                if (name === selected) {
                    ...
                    return { ok: true, name: selected, proof: state.selectionProof, clearBinding: false, verifyBinding: true };
                }
                state.selectionProof = null; state.selectedLabel = "";
            }
        ...
        function verifyPersistence(result, saved, project) {
            if (state.saveIntent === "external") {
                ...
                return !!saved && String(saved.name || "") === String(project && project.name || "")
                    && binding.bound === true && binding.verified === true && saved.fd_work_bound === true;
            }
        ...
        function syncStatus() {
            var capability = App.fdWorkStatus || {}; var input = projectNameInput();
            var picker = document.getElementById("rules-panel-fd-work-picker"); if (picker) picker.hidden = !enabled();
            if (!enabled() || capability.session_state === "shutdown") {
                state.selectionProof = null; state.selectedLabel = ""; state.clearBindingOnSave = false;
                if (state.pickerPending) clearPickerTransient(); showStatus("", false);
            } else if (state.pickerPending) {
                if (state.pickerCanceling) showStatus("正在取消案件选择…", false);
                else if (capability.operation === "user_auth") showStatus(capability.page_phase === "login_confirmation" ? "请确认登录" : "请登录 FD Work", false);
                else if (capability.operation === "user_picker") showStatus("请在 FD Work 中选择案件", false);
                else if (capability.session_state === "error") { clearPickerTransient(); showStatus(App.fdWorkStatusText(capability), true); }
            }
            if (input) input.readOnly = state.pickerPending; refresh();
        }
        function updateControls(projectBusy) {
            state.projectBusy = !!projectBusy;
            var pending = state.pickerPending; var pick = document.getElementById("rules-panel-fd-work-pick");
            var clear = document.getElementById("rules-panel-fd-work-clear"); var input = projectNameInput();
            var name = String(input && input.value || "").trim(); if (input) input.readOnly = pending;
            if (pick) {
                pick.disabled = state.projectBusy || state.pickerCanceling || !enabled();
                pick.textContent = pending ? "取消选择" : (state.selectionProof || state.originalBound ? "更换案件" : "选择案件");
            }
            if (clear) {
                clear.disabled = state.projectBusy || pending; clear.hidden = !(state.selectionProof || state.originalBound);
                clear.textContent = state.selectionProof ? "取消选择" : (state.clearBindingOnSave ? "保持关联" : "取消关联");
            }
        ...
        function bindHost(nextHost) {
            nextHost = nextHost || {};
            host = {
                onStateChanged: typeof nextHost.onStateChanged === "function" ? nextHost.onStateChanged : function () {},
                onBindingChanged: typeof nextHost.onBindingChanged === "function" ? nextHost.onBindingChanged : function () {}
            };
        ...
        function intercept(node, eventName, handler, marker) {
            ...
            node.addEventListener(eventName, function (event) {
                if (event.stopImmediatePropagation) event.stopImmediatePropagation(); handler(event);
            }, true);
        }
        function bindEvents() {
            intercept(document.getElementById("rules-panel-fd-work-pick"), "click", openPicker, "data-fd-work-picker-lifecycle");
            intercept(document.getElementById("rules-panel-fd-work-clear"), "click", clearSelection, "data-fd-work-clear-lifecycle");
            intercept(projectNameInput(), "input", handleNameInput, "data-fd-work-name-lifecycle");
            if (baseIdentity && typeof baseIdentity.bindEvents === "function") baseIdentity.bindEvents();
        }
        function cancelFDWorkCasePicker(requestId) {
            if (!window.pywebview || !window.pywebview.api || typeof window.pywebview.api.cancel_fd_work_case_picker !== "function") {
                return Promise.reject(new Error("bridge method unavailable"));
            }
            return window.pywebview.api.cancel_fd_work_case_picker(requestId);
        }
        function updateProjectForRules(projectId, name, description, language, proof) {
            var explicitClear = state.clearBindingOnSave === true && Number(projectId) === Number(state.editingProjectId) && !proof;
            return Promise.resolve(baseBridge.updateProjectForRules(projectId, name, description, language, proof)).then(function (result) {
                ...
                return Promise.resolve(baseBridge.clearFDWorkBindingForRules(projectId)).then(function (cleared) {
                    if (!cleared || cleared.ok === false) return { ok: false, error: "项目已保存，但取消 FD Work 关联失败" };
                    return Object.assign({}, result, { fd_work_binding: cleared.fd_work_binding || { bound: false } });
                }).catch(function () { return { ok: false, error: "项目已保存，但取消 FD Work 关联失败" }; });
            });
        }

        App.bridge = Object.freeze(Object.assign({}, baseBridge, {
            cancelFDWorkCasePicker: cancelFDWorkCasePicker,
            updateProjectForRules: updateProjectForRules
        }));
        App.projectIdentity = Object.freeze({
            enabled: enabled, bindHost: bindHost, bindEvents: bindEvents, prepareEditor: prepareEditor,
            reset: reset, syncStatus: syncStatus, updateControls: updateControls, buildSavePayload: buildSavePayload,
            verifyPersistence: verifyPersistence, openPicker: openPicker, clearSelection: clearSelection,
            cancelPicker: function () { return cancelPicker(false); }, _testState: function () { return state; }
        });
    ...
    if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", installFDWorkProjectIdentityLifecycle);
    else installFDWorkProjectIdentityLifecycle();
})();
