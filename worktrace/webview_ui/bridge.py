...
# Note: Project Rules error-message maps, payload helpers, and bridge
# methods now live in ``bridge_rules.py`` (Phase M3 split). They are
# re-exported above so existing tests that reference
# ``bridge_module._PROJECT_RULE_...`` continue to resolve.


class WebViewBridge(ProjectRulesBridgeMixin):
    ...
    # --- Phase 5B-5G: Project Rules bridge methods -------------------
    # The 12 Project Rules bridge methods (get_project_rules,
    # set_project_rule_enabled, create/update/delete_project_keyword_rule,
    # create/update/delete_project_folder_rule, create/update_project_for_rules,
    # set_project_enabled_for_rules, archive_project_for_rules) now live in
    # `ProjectRulesBridgeMixin` in `bridge_rules.py` (Phase M3 split).
    # They are inherited here via `class WebViewBridge(ProjectRulesBridgeMixin)`;
    # the method names and Chinese error messages are unchanged.
