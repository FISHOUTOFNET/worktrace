...
from .report_projection_snapshot_service import _compute_projection
...
@dataclass
class _InFlight:
    """Single-flight entry: a Future and the cache epoch it was created in."""
...
def _build_day_projection(
    ...
    then freezes only the entries and contributions needed by page-read
    paths. Indexes are built in O(N) via list accumulation and reference
    the same frozen objects. The full ReportProjectionSnapshot freeze
    (base_sessions, mutually-exclusive subsets) is skipped.
    """
    with stage("projection_compute"):
        comp = _compute_projection(conn, report_date, report_date)

    with stage("projection_materialize"):
        entries = tuple(freeze_value(entry) for entry in comp.final_entries)
        contributions = tuple(
            freeze_value(contribution)
            for contribution in comp.final_contributions
        )
        ...
        with stage("index_build"):
            ...
            # O(N) accumulation: collect into lists, freeze once at the end.
            # Avoids the O(K^2) tuple-rebuild pattern (*existing, item).
            mutable_index: dict[str, list[Mapping[str, Any]]] = {}
            for contribution in contributions:
                key = str(contribution.get("projection_instance_key") or "")
                if key:
                    mutable_index.setdefault(key, []).append(contribution)
            contributions_by_key = FrozenDict(
                {
                    key: tuple(values)
                    for key, values in mutable_index.items()
                }
            )

    # _compute_projection already called record_counts with the real counts.
    return DayProjection(
        report_date=report_date,
        source_version=source_version,
        entries=entries,
        contributions=contributions,
        operation_diagnostics=tuple(comp.operation_diagnostics),
...
def _single_flight_build(
    ...
    with _in_flight_lock:
        ...
        if existing is not None and existing.epoch == current_epoch:
            ...
            future = concurrent.futures.Future()
            _in_flight[flight_key] = _InFlight(future=future, epoch=current_epoch)
            is_builder = True
    ...
    if not is_builder:
        try:
            ...
            raise ProjectionWaitTimeout(
                ...
                f"source_version={source_version.token()} "
                f"timeout={_PROJECTION_WAIT_TIMEOUT}s"
            )
