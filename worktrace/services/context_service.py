    ...
    def build(
        ...
        *,
        ...
        for index, row in enumerate(projected):
            if not _eligible(row, carry_seconds):
                continue
...
def _has_direct_project(row: Mapping[str, Any]) -> bool:
    return (
        ...
        and str(row.get("assignment_source") or "") in DIRECT_ASSIGNMENT_SOURCES
...
def _find_anchor(
    ...
    while 0 <= cursor < len(rows):
        ...
        if _crosses_boundary(left, right, boundaries) or _row_blocks_context(rows[cursor]):
            return None
...
def _row_blocks_context(row: Mapping[str, Any]) -> bool:
    status = str(row.get("status") or "")
    if status == STATUS_PAUSED:
        return True
    return does_status_require_boundary(status, _row_duration_seconds(row))
...
def _row_duration_seconds(row: Mapping[str, Any]) -> int:
    ...
    observed = max(0, int((end - start).total_seconds())) if start and end and end >= start else 0
