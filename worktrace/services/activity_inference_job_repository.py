...
def seed_legacy_import_jobs(conn, *, at_time: str | None = None) -> int:
    """Convert only legacy unresolved assignment semantics into import jobs."""
...
def _safe_error_code(value: str) -> str:
    normalized = str(value or "inference_failed").strip().lower()
    safe = "".join(char if char.isalnum() or char == "_" else "_" for char in normalized)
    return (safe.strip("_") or "inference_failed")[:64]
