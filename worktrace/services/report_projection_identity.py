...
def member_identity_key(
    member: Mapping[str, Any] | ReportMemberIdentity,
    *,
    report_date: str = "",
) -> tuple[str, int, str]:
...
def member_set_hash(report_date: str, members: SequenceMapping) -> str:
    parts = [
        ...
        for key in sorted(
            member_identity_key(member, report_date=report_date)
        ...
        contributions.append(
            {
                ...
                "duration": None
                if is_live
                else int(row.get("duration_seconds") or 0),
                "activity_identity_key": str(
                    row.get("activity_identity_key") or ""
                ),
                "display_project_id": int(row.get("display_project_id") or 0),
                "report_project_id": int(
                    row.get("report_project_id") or row.get("project_id") or 0
                ),
    ...
    return stable_json_hash(
        {
            ...
            "duration": {
                "value": None
                if is_live
                else int(session.get("duration_seconds") or 0),
                "adjusted": None
                if is_live
                else session.get("adjusted_duration_seconds"),
...
def projection_revision(
    ...
    *,
    ...
    """Versioned durable revision used for optimistic write admission."""
