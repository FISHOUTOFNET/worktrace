...
- 当前活动应用名称
- 当前窗口标题
- 当前活动文件的完整本地路径（如可识别），用于文件夹项目规则和项目归类
- 使用开始时间、结束时间和持续时间
...
文件路径可能包含用户名、客户名、项目名、案件名等敏感信息；导出全部本地数据时可能包含这些路径字段，请妥善保存导出文件。

隐私排除关键词会同时匹配应用名称、进程名称、窗口标题和本地文件路径。命中隐私排除后，WorkTrace 只记录“已排除窗口”，不会保存真实窗口标题或文件路径。

如果不希望某些客户、案件、文件夹或个人目录被记录，可以把相关关键词加入隐私排除关键词。
