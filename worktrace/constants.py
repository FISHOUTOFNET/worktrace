...
复制文字记录默认关闭；开启后用于关键词归类和后续本地工作描述生成，有迹会自动清理 30 天前的复制文字。

文件路径和复制文字可能包含用户名、客户名、项目名、案件名等敏感信息；请按需设置排除规则，并只在需要时开启复制文字记录。

排除规则支持文件夹和关键词。关键词会同时匹配应用名称、进程名称、窗口标题和本地文件路径。命中排除规则后，有迹只记录"已排除窗口"，不会保存真实窗口标题、文件路径、邮件标题、浏览器域名或 IDE 信息。
