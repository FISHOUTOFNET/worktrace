...
--color-text-tertiary: #747e8c;
--color-border: #d8dde5;
--color-border-strong: #b7c0cc;
--color-accent: #1769c2;
--color-accent-hover: #0f5aa8;
--color-accent-soft: #e7f1fc;
--color-success: #237447;
--color-success-soft: #eaf6ef;
--color-warning: #8a5a00;
--color-warning-soft: #fff6dd;
--color-danger: #b42318;
--color-danger-hover: #952018;
--color-danger-soft: #fff0ee;
--color-scrim: rgba(22, 29, 37, .34);
--color-focus: #0067c0;
```
...
--radius-lg: 8px;
--border-width: 1px;
--shadow-flyout: 0 8px 28px rgba(24, 32, 43, .18);
--shadow-dialog: 0 16px 44px rgba(24, 32, 43, .24);
--focus-ring: 0 0 0 2px var(--color-surface), 0 0 0 4px var(--color-focus);
```

- 标准页面边距 24px；紧凑窗口为 16px。
- 默认控件高 32px，主要按钮可用 36px，紧凑工具按钮可用 28px。
- 普通表面无阴影；只有 Drawer、Dialog、Menu、Toast 使用阴影。
- 圆角以 6px 为主，外层工作区最多 8px，不使用胶囊形大面积容器。

### 2.4 响应式断点

- `>= 960px`：192–208px 标准导航，Timeline 双栏。
...
使用 8px 间距、浅色表面和底边框，可在窄窗口分行。过滤条件有显式标签，状态与总计靠右但不得被挤出可视区。
