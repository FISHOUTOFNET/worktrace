...
- 固定 30px 高、单行 Flex、2px segment 间隙；不换行且不出现横向滚动。
- 默认 `--overview-bar-min-segment: 88px`，小于 720px 时为 64px；每段使用该固定基础宽度，剩余空间按权威时长权重分配。
...
Overview 的今日总时长与当前活动时长共用距内容右边缘 17px 的视觉轴，
均使用 24px `--font-size-2xl`、700 字重和 `tabular-nums`。最近记录采用
“52px 开始时间 / 可收缩内容 / 84px 持续时长”的稳定三列；左右时间均使用
14px 等宽数字。“进行中”属于内容列的项目标题行，不占独立布局列。
