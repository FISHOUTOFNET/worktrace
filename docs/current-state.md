...
- Project Rules: searchable/sortable summaries, direct actions and Drawers. With FD Work disabled, project names remain ordinary editable local text. With it enabled, new projects use an explicit native FD Work picker and require its one-use selection proof; WorkTrace provides no cross-window autocomplete.
...
Standard CI validates one exact revision. Python diagnostics are artifact-only; repairs are grouped by root cause, and concurrency tests use bounded events/joins plus risk markers. Only `ci.yml` and `_validation.yml` are permanent workflows; historical phases are in [`history/webview-phases.md`](history/webview-phases.md).
