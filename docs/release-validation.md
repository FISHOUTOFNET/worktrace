...
Build the canonical Windows release:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\build_windows_release.ps1
```

The canonical release build uses a temporary `build\release-staging\...` PyInstaller output and publishes only versioned user-facing artifacts to `dist\`.
...
- [ ] `dist\Trace-<version>.exe` is generated.
- [ ] `dist\Trace-Setup-<version>.exe` is generated and embeds the canonical 有迹 icon resource.
- [ ] `dist\Trace.exe` is not generated by the canonical release build.
- [ ] `dist\Trace-Setup.exe` is not generated.
- [ ] No legacy `WorkTrace*.exe` release artifact remains in `dist\`.
...
- [ ] Installed main executable is `Trace.exe`.
- [ ] Portable release is `Trace-<version>.exe`.
- [ ] Installer is `Trace-Setup-<version>.exe`; no unversioned release aliases are shipped.
...
- [ ] Starting the versioned portable executable again activates the existing hidden instance.
...
- [ ] Upgrade can close either the old `WorkTrace.exe` or current installed `Trace.exe` cooperatively before replacement.
...
- Canonical release output contains an unversioned `Trace.exe`, `Trace-Setup.exe`, or a legacy `WorkTrace*.exe` artifact.
