...
function Start-TraceForSmoke {
    # A stale log marker from a prior installed launch must never satisfy the
    # readiness check for the process we are about to start.
...
function Wait-ForTraceWindow {
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        ...
        if ($TargetProcess.HasExited) {
            throw "Installed Trace exited during startup with code $($TargetProcess.ExitCode)."
        }
    ...
    throw "Installed Trace did not load its WebView window within $TimeoutSeconds seconds; startup log: $startupLogHint"
}
...
try {
    $process = Start-TraceForSmoke
    Wait-ForTraceWindow -TargetProcess $process

    if ($KeepRunning.IsPresent) {
        # Prove the maintenance control works before the installer relies on it,
        # then leave a fresh, fully initialized process for upgrade/uninstall.
