...
def replace_function(path: str, name: str, replacement: str) -> None:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"{path}: expected one top-level function {name}, found {len(matches)}"
        )
    node = matches[0]
    lines = source.splitlines(keepends=True)
    replacement_text = block(replacement).rstrip() + "\n\n"
    lines[node.lineno - 1 : node.end_lineno] = [replacement_text]
    write(path, "".join(lines))


write(
    "worktrace/database_failure_policy.py",
    block(
        '''
...
# Inference: keep the historical public integer API, use a result-bearing internal batch.
path = "worktrace/services/activity_inference_job_service.py"
source = read(path)
...
repo_path = "worktrace/services/activity_inference_job_repository.py"
repo = read(repo_path)
marker = "\n\ndef clear_all_jobs(conn) -> int:\n"
if marker not in repo:
    raise RuntimeError("inference repository insertion marker missing")
repo = repo.replace(
    marker,
    block(
        '''
...
# Folder core: infrastructure contention must not become a durable index error.
path = "worktrace/services/folder_index_service.py"
source = read(path)
source = source.replace(
...
if old not in source:
    raise RuntimeError("folder index build exception block missing")
write(path, source.replace(old, new, 1))

# Folder reconciliation: keep old integer API and add truthful result API for runtime.
path = "worktrace/services/folder_index_maintenance_service.py"
source = read(path)
source = source.replace(
...
# Folder runtime is the single production worker-loop owner.
path = "worktrace/services/folder_index_runtime_service.py"
source = read(path)
source = source.replace(
    "from ..constants import EXCLUDED_PROJECT\n",
    "from ..constants import EXCLUDED_PROJECT\n"
...
# Remove only the dangerous lifecycle monkeypatch; preserve the compatibility facade
# for non-lifecycle folder-index entrypoints to minimize unrelated churn.
path = "worktrace/services/__init__.py"
source = read(path)
if source.count('    "run_folder_index_worker",\n') != 1:
    raise RuntimeError("folder worker compatibility export shape changed")
write(path, source.replace('    "run_folder_index_worker",\n', "", 1))

path = "worktrace/runtime/app_runtime.py"
source = read(path)
if source.count("folder_index_service,") != 1:
    raise RuntimeError("AppRuntime folder-index import shape changed")
if source.count("folder_index_service.run_folder_index_worker") != 1:
    raise RuntimeError("AppRuntime folder worker target shape changed")
source = source.replace(
...
# History: transient DB failures are worker retry, not terminal durable job failure.
path = "worktrace/services/history_mutation_job_service.py"
source = read(path)
source = source.replace(
...
# Startup recovery maps the shared infrastructure policy back to its durable enum.
path = "worktrace/services/recovery_service.py"
source = read(path)
source = source.replace(
...
# Resource repair already propagates errors; classify its retry/health code truthfully.
path = "worktrace/services/activity_fact_repair_service.py"
source = read(path)
source = source.replace(
...
# Architecture governance points at the actual folder-index lifecycle owner.
for test_path in (
    "tests/test_architecture_owner_governance.py",
    "tests/test_final_architecture_governance.py",
):
...
write(
    "tests/test_worker_retry_health_contract.py",
    block(
        '''
        from __future__ import annotations
        ...
        def test_folder_build_database_busy_is_not_permanent_error(
            ...
            with pytest.raises(sqlite3.OperationalError, match="database is locked"):
                folder_index_service.rebuild_folder_index(3)
            assert failures == []
...
if not compileall.compile_dir(ROOT / "worktrace", quiet=1):
