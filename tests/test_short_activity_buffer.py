...
def test_project_display_official_only_without_30s_inheritance(temp_db):
    prior = ProjectLabel(name="Manual", id=7, source="manual")
    state = ProjectOwnershipState(
        display_project=prior,
        candidate_project=prior,
        last_confirmed_project=prior,
    )
    official = ProjectLabel(name="Rules", id=8, source="keyword_rule")
    official_state = begin_ownership_for_new_resource(state, official, f"{DATE} 09:00:00")
    assert official_state.display_project == official
    assert official_state.project_transition.pending is False
    assert official_state.project_transition.threshold_seconds == 0

    suggested = ProjectLabel(
        name="Suggested", source="suggested_project_name", is_suggested_project=True
    )
    suggested_state = begin_ownership_for_new_resource(state, suggested, f"{DATE} 09:00:01")
