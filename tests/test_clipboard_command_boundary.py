...
def _bridge(runtime: _ClipboardRuntime) -> WebViewBridge:
    return WebViewBridge(ApplicationControl(runtime))
...
def test_live_runtime_enable_requires_privacy_before_persistence(temp_db):
    ...
    with patch.object(
        ...
        result = _bridge(_ClipboardRuntime(events)).set_clipboard_capture_enabled(True)
...
def test_runtime_rejection_never_persists_preference(temp_db):
    ...
    with patch.object(
        ...
        result = _bridge(
            _ClipboardRuntime(events, accepted=False)
