...
def test_initialize_empty_database_sets_current_schema_version(tmp_path):
    from worktrace.db import CURRENT_SCHEMA_VERSION, initialize_database, get_connection
    ...
    try:
        ...
        assert "source_rule_type" in _get_columns(conn, "activity_project_assignment")
    finally:
        conn.close()
