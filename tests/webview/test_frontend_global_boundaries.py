...
def test_overview_kpi_targets_use_raw_seconds_not_string_parsing() -> None:
    """Overview KPI seeding must use numeric base fields. It must NOT parse
    ``classified_duration`` / ``uncategorized_duration`` HH:MM:SS strings
    to compute live bases."""
    ...
    assert "classified_seconds" in body, (
        "showOverview must read classified_seconds (numeric baseline)"
    )
    assert "uncategorized_seconds" in body, (
        "showOverview must read uncategorized_seconds (numeric baseline)"
    )
    # Must NOT parse the string duration fields to compute the ticker.
    assert "classified_duration" not in body, (
        "showOverview must NOT parse classified_duration strings"
    )
    assert "uncategorized_duration" not in body, (
        "showOverview must NOT parse uncategorized_duration strings"
    )


def test_overview_seeds_classified_and_uncategorized_kpi_targets() -> None:
    """Overview must seed kpi-classified and kpi-uncategorized duration
    targets along with kpi-total, so the generic ticker updates all three
    on the same basis."""
    source = read_js("overview.js")
    pos = source.find("function showOverview")
    assert pos != -1, "overview.js must define function showOverview"
    ...
    assert "kpi-classified" in body, (
        "showOverview must seed kpi-classified DOM element"
    )
    assert "kpi-uncategorized" in body, (
        "showOverview must seed kpi-uncategorized DOM element"
    )
    assert "kpi-total" in body, (
        "showOverview must seed kpi-total DOM element"
    )
