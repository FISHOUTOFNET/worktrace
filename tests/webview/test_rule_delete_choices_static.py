...
def test_rule_delete_dialog_exposes_preserve_and_recompute_history_modes():
    ...
    assert 'value: "recompute"' in source
    assert 'label: "重新计算已有归类"' in source
    assert 'defaultChoice: "preserve"' in source
    assert 'deleteRule(kind, ruleId, true)' in source
    assert 'deleteRule(kind, ruleId, false)' in source
    assert "没有其他规则匹配时变为“未归类”" in source
