...
def test_explicit_writer_hold_is_attributed_without_sql_or_parameters(
    temp_db,
    monkeypatch,
    caplog,
):
    ...
    message = next(
        ...
        if "database writer slow" in record.getMessage()
    )
...
def test_database_busy_logs_waiter_contention_at_connection_boundary(
    temp_db,
    caplog,
):
    caplog.set_level(logging.WARNING)
    ...
    message = next(
        record.getMessage()
        for record in caplog.records
        if "database writer contention" in record.getMessage()
    )
