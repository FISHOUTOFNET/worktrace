...
def test_projection_edit_api_rejects_stale_revision(temp_db):
    ...
    with pytest.raises(Exception, match="revision_conflict"):
        timeline_api.save_timeline_session_edit(
            ...
            "edit-stale", None, False, None, "",
        )


@pytest.mark.parametrize("bad_touched", [None, 0, 1, "true", [], {}])
def test_timeline_api_requires_strict_duration_touched_bool(temp_db, bad_touched):
    source = _session()
...
def test_timeline_api_ignores_duration_value_when_not_touched(temp_db):
    source = _session()
    ...
    assert result["ok"] is True
    updated = _session_for_existing_project()
    assert updated["session_note"] == "note only"
    assert updated["has_duration_override"] is False


def _session_for_existing_project():
    return timeline_api.get_project_sessions_by_date(DATE)[0]
