"""Cross-layer behavior regressions for current architecture invariants.

These tests exercise durable behavior. Historical source-name blacklists live
nowhere: once a migration is complete, only the surviving product invariant is
kept here.
"""
...
def test_keyword_normalizer_is_durable_and_unique(temp_db) -> None:
...
def test_keyword_uniqueness_applies_to_direct_command_and_self_update(temp_db) -> None:
...
def test_same_normalized_keyword_is_allowed_in_different_projects(temp_db) -> None:
...
def test_concurrent_duplicate_keyword_write_has_one_durable_winner(temp_db) -> None:
...
def test_project_lifecycle_invariants_are_service_owned(temp_db) -> None:
...
def test_archived_project_cannot_be_new_rule_target(temp_db) -> None:
...
def test_privacy_evaluation_is_read_only_and_fail_closed(temp_db, monkeypatch) -> None:
    monkeypatch.setattr(
        privacy_service,
        "get_connection",
...
def test_live_clock_contract_is_exact_key_set() -> None:
...
def test_runtime_start_result_is_worker_mapping_only() -> None:
...
def test_collector_maintenance_hold_has_explicit_terminal_states() -> None:
...
def test_view_model_api_requires_explicit_runtime_context() -> None:
